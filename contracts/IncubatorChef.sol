...
contract IncubatorChef is Ownable, ReentrancyGuard, IIncubatorChef  {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;        // Last block number that KSWAPs distribution occurs.
        uint256 accKswapPerShare;       // Accumulated KSWAPs per share, times 1e12. See below.
    ...
    // The block number when KSWAP mining starts.
    ...
    // Update the given pool's KSWAP allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending KSWAPs on frontend.
    function pendingKswap(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accKswapPerShare = accKswapPerShare.add(kswapReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accKswapPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // Deposit tokens to chef for KSWAP allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accKswapPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeKswapTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accKswapPerShare).div(1e12);
    ...
    function safeKswapTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "safeKswapTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _kswapPerBlock) external onlyOwner {
        _massUpdatePools();
        kswapPerBlock = _kswapPerBlock;
        emit UpdateEmissionRate(msg.sender, _kswapPerBlock);
    }
    ...
    function harvestFor(uint256 _pid, address _user) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accKswapPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeKswapTransfer(_user, pending);
                user.rewardDebt = user.amount.mul(pool.accKswapPerShare).div(1e12);
