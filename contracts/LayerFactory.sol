...
contract LayerFactory is Ownable, ReentrancyGuard, BscConstants {
    ...
    uint256 public pricePerKswap = 1;
    ...
    event AddLiquidity(uint256 indexed layerId, address indexed kswapToken, address indexed priceToken, uint256 kswapAmount, uint256 priceAmount);
    event RemoveLiquidity(uint256 indexed layerId, address indexed kswapToken, address indexed priceToken, uint256 lpAmount);
    event SetFeeHolder(address indexed user, address feeHolder);
    event SetKswapHolder(address indexed user, address kswapHolder);
    ...
    function addLiquidityBusd(uint256 layerId, IMintable token) private {
        ...
        uint256 busdAmount = tokenAmount.mul(pricePerKswap);
    ...
    function spawnNewFeeProcessor(uint256 layerId, address houseChef, address kswapToken) private returns (IFeeProcessor){
    ...
    function removeBUSDLiquidity(uint256 layerId) external onlyAdmins validLayer(layerId) nonReentrant {
        ...
        address factory = IPancakeRouter01(routerAddr).factory();
        address kswapToken = address(layerInfo.token);
        address lpToken = PancakeLibrary.pairFor(factory, kswapToken, busdAddr);
        uint256 balance = IBEP20(lpToken).balanceOf(address(this));
        IBEP20(lpToken).safeApprove(routerAddr, balance);
        IPancakeRouter01(routerAddr).removeLiquidity(kswapToken, busdAddr, balance, 0, 0, address(this), getTxDeadline());

        emit RemoveLiquidity(layerId, kswapToken, busdAddr, balance);
    }
    ...
    function removeWBNBLiquidity(uint256 layerId) external onlyAdmins validLayer(layerId) nonReentrant {
        ...
        address factory = IPancakeRouter01(routerAddr).factory();
        address kswapToken = address(layerInfo.token);
        address lpToken = PancakeLibrary.pairFor(factory, kswapToken, wbnbAddr);
        uint256 balance = IBEP20(lpToken).balanceOf(address(this));
        IBEP20(lpToken).safeApprove(routerAddr, balance);
        IPancakeRouter01(routerAddr).removeLiquidity(kswapToken, wbnbAddr, balance, 0, 0, address(this), getTxDeadline());

        emit RemoveLiquidity(layerId, kswapToken, wbnbAddr, balance);
    }
    ...
    function setKswapHolder(address newAddr) external nonReentrant {
        require(msg.sender == kswapHolder, "setKswapHolder: FORBIDDEN");
        kswapHolder = newAddr;

        emit SetKswapHolder(msg.sender, newAddr);
    }
